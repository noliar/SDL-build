...
& "$env:APPVEYOR_BUILD_FOLDER\nuget.exe" @('pack', 'build\DevZH.SDL.Internal-UWP.nuspec', '-BasePath', "$env:APPVEYOR_BUILD_FOLDER\output\uwp", '-Version', '"2.0.4.1"', '-Suffix', """nightly-$rev""", '-Verbosity', 'detailed');
...
& "$env:APPVEYOR_BUILD_FOLDER\nuget.exe" @('pack', 'build\DevZH.SDL.Internal-Windows.nuspec', '-BasePath', "$env:APPVEYOR_BUILD_FOLDER\output\win", '-Version', '"2.0.4.1"', '-Suffix', """nightly-$rev""", '-Verbosity', 'detailed');
